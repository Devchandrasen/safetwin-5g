...
void RlsControlTask::handleRlsMessage(int cellId, rls::RlsMessage &msg)
{
    ...
    else if (msg.msgType == rls::EMessageType::PDU_TRANSMISSION)
    {
        ...
        if (m.pduId != 0 && m_pendingAck[cellId].size() < MAX_PDU_COUNT)
...
{
// SAFETWIN_R3_BEGIN ue-rls
    safetwin_r3::emit(m_logger.get(), data, "ue_rls", psi, m_servingCell, -1, -1, -1, -1);
// SAFETWIN_R3_END ue-rls
    rls::RlsPduTransmission msg{m_shCtx->sti};

...
SCOPE_PATH = ROOT / "config/experiments/reconnect-r3-scope-reference.json"
REQUIRED_SOURCES = {
    "sandbox/run_reconnect_r3.py", "sandbox/reconnect_r3_measurement.py",
    "sandbox/compose.reconnect-r3.yaml", "sandbox/compose.yaml", "sandbox/versions.lock.json",
    "sandbox/run_reconnect.py", "sandbox/run_recovery_pilot.py", "config/actions.json",
    "config/experiments/reconnect-r3-trace.json", "config/experiments/reconnect-r3-images.json",
    "config/experiments/reconnect-r3-command-contract.json", "config/experiments/reconnect-r3-scope-reference.json",
    "tools/reconnect_r3_trace.py", "tools/audit_reconnect_r3_network.py",
    "tests/reconnect_r3_fixture.py", "tests/test_reconnect_r3_execution.py",
}
...
def verify_execution_lock():
    ...
    if lock.get("lock_id") != "reconnect-r3-execution-v1" or not REQUIRED_SOURCES <= set(lock.get("source_sha256", {})):
        raise PermissionError("complete R3 execution inventory required")
...
class R3Backend:
    def __init__(self, output, config, images, approval, clock=None):
        ...
        self.reference = json.loads(SCOPE_PATH.read_bytes())["containers"]

    def invoke(self, argv, merged, timeout):
        return subprocess.run(argv, cwd=ROOT, stdout=subprocess.PIPE, stderr=subprocess.STDOUT if merged else subprocess.PIPE, timeout=timeout)

    def cmd(self, name, argv, accepted=(0,), merged=False, timeout=35):
        if not 0 < timeout <= 35: raise ValueError("command timeout outside frozen bound")
        ...
        row = {"sequence": self.sequence, "unit_id": self.unit_id, "name": name, "argv": argv,
               "started_at": self.now(), "timeout_seconds": timeout, "merged_streams": merged}
        try:
            result = self.invoke(argv, merged, timeout)
            ...
            code, stdout, stderr = -998, b"", (type(exc).__name__ + ": " + str(exc)).encode()
    ...
    def health(self, container):
        ...
        while True:
            remaining = until - self.monotonic()
            if remaining <= 0: raise TimeoutError("health wait exhausted: " + container)
            status = self.cmd("health-" + container, ["docker", "inspect", "--format", "{{.State.Health.Status}}", container], timeout=min(35, remaining)).strip()
            if status == "healthy": return
            self.sleep(min(2, max(0, until - self.monotonic())))
...
def main():
    ...
    try:
        ...
        save(output / "design.json", {"config": config, "images": images, "source_sha256": lock["source_sha256"], "execution_mode": "sandbox", "transport": "docker-cli"})

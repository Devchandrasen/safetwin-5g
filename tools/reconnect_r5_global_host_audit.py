...
def audit(host, snapshot, raw_journal, receipt, global_context):
    clock_audit = global_context["journal"]
    ...
    if idle is not None:
        ...
        need(idle == global_context["rows"][0], "idle global row linkage")
        pa = global_context["process"]
        acquire = ops[0]
        following = ops[1]

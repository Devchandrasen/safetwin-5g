...
def audit(windows, snapshot, raw_journal, ledger_snapshot, raw_ledger):
    check(type(windows) is list and 1 <= len(windows) <= 99, "collection inventory")
    j = journal_audit(snapshot, raw_journal)
    points, events, f = snapshot["points"], snapshot["events"], snapshot["descriptor"]["frequency_hz"]
    reservations = ledger_replay(ledger_snapshot, raw_ledger, snapshot["descriptor"]["clock_id"])
    ...
    process = process_audit(rows, snapshot, raw_journal, allowed, allowed) if rows else None
    ...
    for index, (w, reservation) in enumerate(zip(windows, reservations), 1):
        ...
        check(w["start_event"] == previous_end if index > 1 else all(e["kind"] == "point" and str(e.get("label", "")).startswith("bootstrap:") for e in events[1:w["start_event"]]), "standalone collection journal inventory")
        ...
        previous_end = b
    check(previous_end == len(events), "unaccounted trailing journal work")

"""Retained synthetic collection gate; no real diagnostic or native probe."""
...
SOURCES = ("sandbox/reconnect_r5_collection.py", "tools/audit_reconnect_r5_collection.py", "tests/test_reconnect_r5_collection.py",
           "tools/verify_reconnect_r5_collection.py", "docs/RECONNECT_R5_COLLECTION.md")
LOCK = "config/experiments/reconnect-r5-collection-lock.json"
PRIOR = "config/experiments/reconnect-r5-host-lock.json"


def verify_lock(committed=False):
    from tools.verify_reconnect_r5_host import verify_lock as host_lock
    host_lock(committed=True)
    lock = json.loads((ROOT/LOCK).read_bytes())
    assert set(lock) == {"lock_id", "source_sha256", "immutable_host_lock_sha256", "network_execution_authorized", "whole_protocol_verified"}
    assert lock["lock_id"] == "safetwin5g-reconnect-r5-collection-v1"
    assert lock["network_execution_authorized"] is False and lock["whole_protocol_verified"] is False
    assert set(lock["source_sha256"]) == set(SOURCES) and lock["immutable_host_lock_sha256"] == sha((ROOT/PRIOR).read_bytes())
    for name in SOURCES:
...
def replay_fresh(bundle, directory):
    from tools.audit_reconnect_r5_collection import audit, packet_candidate
    from tests.test_reconnect_r5_collection import check_case
    check_case(bundle)
    args = (bundle["windows"], bundle["snapshot"], (directory/"clock.jsonl").read_bytes(), bundle["ledger"], (directory/"identifiers.jsonl").read_bytes())
    if bundle["audit_error"]:
        try:
            ...
            raise ValueError("failed collection integrity accepted")
    else:
        report = audit(*args)
        assert report == bundle["independent"]
        assert packet_candidate(bundle["windows"], report) == bundle["independent_candidate"]
...
def main():
    ...
    from tests.test_reconnect_r5_collection import capture_case, CASES
    output = ROOT/"evidence/verification"/(datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")+"-reconnect-r5-collection")
    ...
    if not args.development:
        try:
            verify_lock()
            checks["immutable-and-collection-locks"] = dict(passed=True)
        except (ValueError, AssertionError, OSError) as exc:
            checks["immutable-and-collection-locks"] = dict(passed=False, error=str(exc))
        checks["environment-before"] = environment(output, "before")
    definitions = {"tests": ([sys.executable, "-m", "pytest", "-q", *(["tests/test_reconnect_r5_collection.py"] if args.development else [])], 300)}
    if not args.development:
        definitions.update({
    ...
    for case in CASES:
        with tempfile.TemporaryDirectory(prefix="safetwin-r5-collection-") as temporary:
    ...
    report = dict(verification_passed=all(c["passed"] for c in checks.values()), development_only=args.development,
                  checks=checks, cases=cases, source_sha256=hashes, evidence_label="fixture", measurement_scope="synthetic-collection-only",

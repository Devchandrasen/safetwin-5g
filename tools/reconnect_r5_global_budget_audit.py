...
def audit(b, snapshot, raw_journal, global_context):
    journal = global_context["journal"]
    ...
    process = None
    need(all(r == global_context["rows"][r["sequence"]-1] for r in rows), "budget global row linkage")
    ...
    for number, w in enumerate(b["operations"], 1):
        ...
        ev = w["start_event"]
        need(integer(ev, 1, len(events)) and (last_event is None or ev >= last_event), "global timing event order")
        n = n_at(ev)
        ...
        seq = 1+sum(r["journal_event_after"] <= ev for r in global_context["rows"])
        ...
        last_event = ev+1
    need(b["counter_error"] == counter or counter is None and global_context["final_ticks"] is None and type(b["counter_error"]) is str and b["counter_error"], "global terminal/latch")

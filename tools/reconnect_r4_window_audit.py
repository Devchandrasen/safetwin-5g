"""Versioned independent R4 raw-window replay for execution integration.

Derived from the frozen independent collection auditor; only evidence-tier and
reported-result wrappers change. No runtime acceptance predicate is imported.
The original fixture-only auditor remains immutable. Whole-protocol approval,
telemetry and command linkage must be independently checked by its caller.
"""
import argparse
...
def replay(bundle, *, allow_fixture=False):
    fixture = bundle["evidence_label"] == "fixture"
    check(bundle["evidence_label"] in ("fixture", "sandbox-measured") and (not fixture or allow_fixture), "explicit evidence-tier permission")
    check(bundle["contract_id"] == C["contract_id"] and bundle["transport"] == ("in-memory-no-io" if fixture else "bounded-windows-job")
          and bundle["actual_docker_commands_executed"] == (0 if fixture else len(bundle["commands"]))
          and bundle["network_execution_authorized"] is (not fixture) and bundle["network_fix_validated"] is False, "claim boundary")
    ...
    check(len(rows) == 15, "incomplete collection")

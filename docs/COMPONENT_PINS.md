...
|---|---|---|---|
| Open5GS | `v2.7.7`, commit `318eeb49a7dcdff733dec60e02d9c60aefca2fb9` | Latest official Open5GS release observed on the selection date; built from the official repository | UE registration and PDU Session ID 1 passed in the software stack |
| UERANSIM | `v3.3.0`, commit `6bf5a1a96aaef6ae8778b9d8b477ac6e2bbf8156` | Latest official UERANSIM release observed on the selection date; built from the official repository | Open5GS interoperation passed; radio claim remains simulated |
| MongoDB | `mongo:8.0.29-noble@sha256:021b2d5a...406e` | Docker Official Image, fixed patch tag and Linux/amd64 digest | Subscriber schema lookup and Open5GS access passed |
| Prometheus | `prom/prometheus:v3.13.2@sha256:508729e0...8b69` | Official supported 3.13 LTS line, fixed patch tag and Linux/amd64 digest | AMF, SMF, and UPF targets healthy in the isolated network |
| Open5GS build/runtime base | `ubuntu:22.04@sha256:2edbbc5d...7dbc` | Docker Official Image, fixed Linux/amd64 digest | Selected after the Debian 12/libcurl compatibility check failed |
| UERANSIM build/runtime base | `debian:bookworm-slim@sha256:abd67ffc...9241` | Docker Official Image, fixed Linux/amd64 digest | Image identity and UERANSIM binary verified locally |
...
- A first source build on Debian 12 (`bookworm-slim`) registered each Open5GS
  network function with NRF but then lost every heartbeat with libcurl error
  16 (`Error in the HTTP2 framing layer`). This matches upstream Open5GS
  discussion #2226. The failed result is retained as a compatibility finding;
  the Open5GS base is now pinned to Ubuntu 22.04, where upstream users report
  the SBI path working. Runtime verification is still pending.
...
- Open5GS discussion #2226 (Debian 12/libcurl SBI failure): https://github.com/open5gs/open5gs/discussions/2226
...
Stack capture -> evidence/sandbox/20260824T044704Z-stack (all checks true)
```

The passing pre-intervention capture is labelled `simulated`, not
`sandbox-measured`, because the project claim gate also requires a logged fault,
approved action, and verified rollback. A successful image build alone was not
used as a compatibility result.
